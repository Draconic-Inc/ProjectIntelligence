...
public class GuiStyleEditor extends GuiPopUpDialogBase<GuiStyleEditor> {
    ...
    public void addChildElements() {
        ...
        addChild(new GuiLabel(TextFormatting.UNDERLINE + I18n.get("pi.style.edit_style_properties.txt"))//
        ...
        //Presets
        int selOffst = 20;
        addChild(new GuiLabel(I18n.get("pi.style.load_preset.txt")).setPos(this).setSize(xSize(), 10).translate(4, selOffst)//
        ...
        addChild(new GuiLabel(I18n.get("pi.style.save_overwrite_preset.txt")).setPos(xPos() + 4, presetBackground.maxYPos() + 3).setSize(xSize() - 10, 10)//
                .setHoverableTextCol(hovering -> StyleHandler.getColour("user_dialogs." + TEXT_COLOUR.getName()).rgb())//
                .setShadow(false).setAlignment(GuiAlign.LEFT).addToGroup("PRESETS"));
        ...
        GuiButton savePreset = new StyledGuiButton("user_dialogs.button_style").setText(I18n.get("pi.button.save")).setPos(saveName.maxXPos() + 1, saveName.yPos()).setSize(41, 14).addToGroup("PRESETS");
        savePreset.setHoverText(I18n.get("pi.style.save_preset.info"));
        savePreset.onPressed(() -> {
            if (saveName.getText().isEmpty()) {
                GuiPopupDialogs.createDialog(this, GuiPopupDialogs.DialogType.OK_OPTION, I18n.get("pi.style.save_no_name.txt"), "").showCenter(displayZLevel + 50);
                return;
            }
            if (StyleHandler.getCustomPresets().contains(saveName.getText())) {
                GuiPopupDialogs.createDialog(this, GuiPopupDialogs.DialogType.YES_NO_OPTION, I18n.get("pi.style.save_overwrite.txt"), "").setYesListener(() -> {
        ...
        GuiButton openEditor = new StyledGuiButton("user_dialogs.button_style").setText(I18n.get("pi.button.open_style_editor")).setPos(xPos() + 4, maxYPos() - 18).setSize(xSize() - 8, 14);
        ...
        GuiButton closeEditor = new StyledGuiButton("user_dialogs.button_style").setText(I18n.get("pi.button.close_style_editor")).setPos(xPos() + 3, maxYPos() - 15).setSize(xSize() - 6, 12);
    ...
    public void reloadElement() {
        ...
        for (String preset : StyleHandler.getCustomPresets()) {
            ...
            button.onPressed(() -> {
                if (StyleHandler.unsavedChanges) {
                    GuiPopupDialogs.createDialog(this, GuiPopupDialogs.DialogType.OK_CANCEL_OPTION, I18n.get("pi.style.confirm_load_unsaved.txt") + "\n" + preset, "")//
            ...
            delete.onPressed(() -> GuiPopupDialogs.createDialog(this, GuiPopupDialogs.DialogType.YES_NO_OPTION, I18n.get("pi.style.delete_preset_confirm.txt") + "\n" + preset, I18n.get("pi.style.confirm_delete.txt"))//
                    .setYesListener(() -> {
                        StyleHandler.deletePreset(preset);
                        reloadElement();
                    }).showCenter(displayZLevel + 50));
            delete.setHoverText(I18n.get("pi.button.delete"));
        ...
        for (String preset : StyleHandler.getDefaultPresets()) {
            GuiButton button = new StyledGuiButton("user_dialogs.sub_elements.button_style").setShadow(false).setText(I18n.get("pi.style.default." + preset)).setYSize(12).setAlignment(GuiAlign.LEFT);
            button.addChild(new GuiLabel(I18n.get("pi.style.builtin")).setShadow(false).setYSize(12).bindSize(button, false).setAlignment(GuiAlign.RIGHT).setHoverableTextCol(hovering -> StyleHandler.getInt("user_dialogs.sub_elements.button_style.text_colour")));
            button.onPressed(() -> {
                if (StyleHandler.unsavedChanges) {
                    GuiPopupDialogs.createDialog(this, GuiPopupDialogs.DialogType.OK_CANCEL_OPTION, I18n.get("pi.style.confirm_load_unsaved.txt") + "\n" + preset, "")//
    ...
    public class StyleSetting extends GuiElement<StyleSetting> {
        ...
        public void addChildElements() {
            if (hasSubs) {
                ...
                showHide.setHoverText(element -> I18n.get(subsShown ? "pi.style.hide_subs.txt" : "pi.style.show_subs.txt"));
            ...
            if (property.isColour()) {
                tt.add(I18n.get("pi.style.set_colour.txt"));
                tt.add(TextFormatting.DARK_GRAY + I18n.get("pi.style.right_click_options.txt"));
            } else if (property.isInteger()) {
                tt.add(I18n.get("pi.style.set_value.txt"));
                tt.add(TextFormatting.DARK_GRAY + I18n.get("pi.style.right_click_options.txt"));
            } else if (property.isBoolean()) { tt.add(I18n.get("pi.style.set_boolean.txt")); } else if (hasSubs) tt.add(I18n.get("pi.style.set_edit_sub_values.txt"));
        ...
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            ...
            if (mouseOver && button == 0) {
                ...
                int rely = 0;
                GuiButton copy = new GuiButton(I18n.get("pi.button.copy_value")).setAlignment(GuiAlign.LEFT).setRelPos(1, rely += 1).setSize(98, 12).setBorderColours(0, 0xFF707070);
                copy.onPressed(() -> {
                ...
                GuiButton paste = new GuiButton(I18n.get("pi.button.paste_value")).setAlignment(GuiAlign.LEFT).setRelPos(1, rely += 13).setSize(98, 12).setBorderColours(0, 0xFF707070);
                paste.onPressed(() -> {
                    try {
                ...
                if (property.isColour()) {
                    GuiButton copyHex = new GuiButton(I18n.get("pi.button.copy_hex_value")).setAlignment(GuiAlign.LEFT).setRelPos(1, rely += 13).setSize(98, 12).setBorderColours(0, 0xFF707070);
                    copyHex.onPressed(() -> {
                    ...
                    GuiButton pasteHex = new GuiButton(I18n.get("pi.button.paste_hex_value")).setAlignment(GuiAlign.LEFT).setRelPos(1, rely += 13).setSize(98, 12).setBorderColours(0, 0xFF707070);
                    pasteHex.onPressed(() -> {
                        try {
                    ...
                    GuiButton lighten = new GuiButton(I18n.get("pi.button.lighten")).setAlignment(GuiAlign.LEFT).setRelPos(1, rely += 13).setSize(98, 12).setBorderColours(0, 0xFF707070);
                    lighten.onPressed(() -> {
                    ...
                    GuiButton darken = new GuiButton(I18n.get("pi.button.darken")).setAlignment(GuiAlign.LEFT).setRelPos(1, rely + 13).setSize(98, 12).setBorderColours(0, 0xFF707070);
                    darken.onPressed(() -> {
                        Colour colour = ((ColourProperty) property).getColour();
                        ((ColourProperty) property).set(changeShade(colour.argb(), -0.05));
                    });
        ...
        public void renderElement(Minecraft minecraft, int mouseX, int mouseY, float partialTicks) {
            ...
            //Draw Text
            drawString(fontRenderer, I18n.get("pi.style." + property.getType().getName() + ".prop"), xPos() + 12, yPos() + 2, 0);
        ...
        public boolean renderOverlayLayer(Minecraft minecraft, int mouseX, int mouseY, float partialTicks) {
            ...
            if (property.isColour() && GuiHelper.isInRect(maxXPos() - 22, yPos() + 1, 10, 10, mouseX, mouseY)) {
                Colour colour = ((ColourProperty) property).getColour();
                tt.add(I18n.get("pi.style.colour_value.txt"));
                ...
                tt.add(I18n.get("pi.style." + (((colour.a & 0xFF) < 255) ? ((colour.a & 0xFF) == 0) ? "transparent" : "semi_transparent" : "opaque") + ".txt"));
            } else if ((property.isInteger() && !property.isColour()) && GuiHelper.isInRect(maxXPos() - 45, yPos() + 1, 33, 10, mouseX, mouseY)) {
                tt.add(I18n.get("pi.style.integer_value.txt"));
