...
import com.brandon3055.projectintelligence.client.gui.swing.UIEditTextArea;
...
//    public int timer = 0;
    ...
    public DCTextArea(GuiProjectIntelligence modularGui, String componentType, TreeBranchRoot branch) {
...
//    }
    ...
    public LinkedList<MGuiElementBase> getEditControls() {
        ...
        list.add(new MGuiButtonSolid("OPEN_EDITOR", 0, 0, 66, 12, "Show Editor") {
            @Override
            public int getBorderColour(boolean hovering, boolean disabled) {
                return hovering ? 0xFF00FF00 : 0xFFFF0000;
            }
        }.setListener(this).setHoverText("Open the editor or bring it to the front if it is already open in the background"));
        return list;
    }
    ...
    public void onMGuiEvent(GuiEvent event, MGuiElementBase eventElement) {
        ...
        if (eventElement instanceof GuiButton && ((GuiButton) eventElement).buttonName.equals("OPEN_EDITOR")) {
...
//            SwingHelper.centerOnMinecraftWindow(editTextArea);
            ...
            try {
                setColour(Integer.parseInt(((GuiTextField) eventElement).getText(), 16));
...
//        branch.guiWiki.contentWindow.setEditingComponent(null);
    ...
    @Override
    public DCTextArea setXSize(int xSize) {
        super.setXSize(xSize);
        if (this.xSize() < 40) {
            setYSize(40);
        }
        setYSize(Math.max(parseRawText().size() * fontRenderer.FONT_HEIGHT, 8));
        return this;
    }
